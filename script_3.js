function Prover(initFormulas, parser, accessibilityConstraints) {
  ...
  if (parser.isMod) {
    ...
    for (var i = 0; i < initFormulas.length; i++) {
      this.initFormulasNonModal.push(
        this.parser.translateFromModal(initFormulas[i])
      );
    }
...
Prover.prototype.nextStep = function () {
  ...
  if (todo) {
    ...
    if (this.tree.openBranches[0].nodes.length > this.depLim * 4) {
      this.limitReached();
    }
  }
  ...
  } else if (this.pLen && timeSinceBreak > this.compLen) {
    setTimeout(
      function () {
        this.lastBreakTime = performance.now();
        this.nextStep();
      }.bind(this),
      (this.pLen * this.tree.numNodes) / 100
    );
  } else {
    this.nextStep();
  }
...
Prover.alpha.toString = function () {
  return 'alpha';
};
...
Prover.beta.toString = function () {
  return 'beta';
};
Prover.gamma = function (branch, nodeList, matrix) {
  var node = nodeList[0];
  if (branch.freeVariables.length == this.depLim) {
    this.limitReached();
    return null;
  }
...
Prover.gamma.toString = function () {
  return 'gamma';
};
...
Prover.modalGamma = function (branch, nodeList) {
  ...
  OUTERLOOP: for (var i = 0; i < branch.literals.length; i++) {
    ...
    if (wRy.predicate == wRx.predicate && wRy.terms[0] == wRx.terms[0]) {
      for (var j = 0; j < branch.nodes.length; j++) {
        if (
          branch.nodes[j].fromRule == Prover.modalGamma &&
      ...
      var newNode = new Node(newFormula, Prover.modalGamma, [
        node,
        branch.literals[i],
      ]);
...
Prover.modalGamma.toString = function () {
  return 'modalGamma';
};
...
Prover.delta.toString = function () {
  return 'delta';
};
...
Prover.modalDelta = function (branch, nodeList) {
  ...
  var fla1 = node.formula.matrix.sub1.substitute(
    node.formula.variable,
    newWorldName
  );
  var fla2 = node.formula.matrix.sub2.substitute(
    node.formula.variable,
    newWorldName
  );
...
Prover.modalDelta.toString = function () {
  return 'modalDelta';
};
...
Prover.reflexivity.toString = function () {
  return 'reflexivity';
};
...
Prover.symmetry = function (branch, nodeList) {
  ...
  var R = branch.tree.parser.R;
  var formula = new AtomicFormula(R, [
    nodeFormula.terms[1],
    nodeFormula.terms[0],
  ]);
...
Prover.symmetry.toString = function () {
  return 'symmetry';
};
...
Prover.euclidity = function (branch, nodeList) {
  ...
  for (var i = 0; i < branch.nodes.length; i++) {
    ...
    if (earlierFla.terms[0] == nodeFla.terms[0]) {
      var newFla;
      if (newFla) {
        var newNode = new Node(newFla, Prover.euclidity, [
          branch.nodes[i],
          node,
        ]);
        if (branch.addNode(newNode)) {
      ...
      if (!flaStrings.includes(R + earlierFla.terms[1] + nodeFla.terms[1])) {
        ...
        !flaStrings.includes(R + nodeFla.terms[1] + earlierFla.terms[1])
      ) {
...
Prover.euclidity.toString = function () {
  return 'euclidity';
};
Prover.transitivity = function (branch, nodeList) {
  ...
  for (var i = 0; i < branch.nodes.length; i++) {
    ...
    if (newFla) {
      var newNode = new Node(newFla, Prover.transitivity, [
        branch.nodes[i],
        node,
      ]);
      if (branch.addNode(newNode)) {
...
Prover.transitivity.toString = function () {
  return 'transitivity';
};
...
Prover.seriality.toString = function () {
  return 'seriality';
};
...
Tree.prototype.pruneAlternatives = function () {
  ...
  ALTLOOP: for (var i = 0; i < alternatives.length; i++) {
...
Tree.prototype.containsOpenBranch = function (branch) {
  BRANCHLOOP: for (var i = 0; i < this.openBranches.length; i++) {
...
Tree.prototype.markUsedNodes = function (
  branch,
  complementary1,
  complementary2
) {
...
Tree.prototype.closeCloseableBranches = function () {
  ...
  for (var k = 0; k < openBranches.length; k++) {
    ...
    BRANCHLOOP: for (var i = branch.nodes.length - 1; i >= 0; i--) {
...
Tree.prototype.toString = function () {
  for (var i = 0; i < this.closedBranches.length; i++) {
    this.closedBranches[i].nodes[
      this.closedBranches[i].nodes.length - 1
    ].__markClosed = true;
  }
  var branches = this.closedBranches.concat(this.openBranches);
  var res =
    "<table><tr><td align='center'>" +
    getTree(branches[0].nodes[0]) +
    '</td></tr></table>';
  for (var i = 0; i < this.closedBranches.length; i++) {
    delete this.closedBranches[i].nodes[this.closedBranches[i].nodes.length - 1]
      .__markClosed;
  }
  return res;
  ...
  function getTree(node) {
    ...
    if (++recursionDepth > 100)
      return '<b>......<br>Reached MAX Recursion limit.</b>';
    var children = [];
    ...
    var res =
      (node.used ? '.' : '') +
      node +
      (node.__markClosed ? '<br><br><br>-<br>' : '<br><br><br>');
    if (children[1]) {
      var tdStyle =
        'font-family:monospace; border-top:1px solid #999; padding:3px; border-right:1px solid #999';
      var td = "<td align='center' valign='top' style='" + tdStyle + "'>";
      res +=
        '<table><tr>' +
        td +
        getTree(children[0], recursionDepth) +
        '</td>\n' +
        td +
        getTree(children[1], recursionDepth) +
        '</td>\n</tr></table>';
    ...
    for (var b = 0; b < branches.length; b++) {
      for (var n = branches[b].nodes.length - 1; n >= 0; n--) {
        ...
        if (node.instanceTerm) {
          node.instanceTerm = AtomicFormula.substituteInTerm(
            node.instanceTerm,
            sub[i],
            sub[i + 1]
          );
...
Branch.prototype.tryClose = function (node) {
  var negatedFormula =
    node.formula.operator == '¬' ? node.formula.sub : node.formula.negate();
  ...
  var unifier = unifiers[0],
    otherNode = otherNodes[0];
  VARLOOP: for (var i = 0; i < unifier.length; i += 2) {
...
Branch.prototype.addAccessibilityRuleApplications = function (node) {
  for (var i = 0; i < this.tree.prover.accessRules.length; i++) {
    var rule = this.tree.prover.accessRules[i];
    for (var j = 0; j < this.todoList.length; j++) {
      if (rule.priority <= this.todoList[j][0].priority) break;
    }
    if (node) {
      if (
        node.formula.terms[0] != node.formula.terms[1] ||
