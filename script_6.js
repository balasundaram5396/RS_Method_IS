...
TPaint.prototype.makeContainer = function (node) {
  var parContainer = node.parent ? node.parent.container : this.treeVal;
  ...
  container.style.top = node.parent
    ? parContainer.h + this.branchHeight + 'px'
    : '0px';
...
TPaint.prototype.branchRepo = function (node) {
  ...
  while ((parent = parent.parentNode).subContainers) {
    ...
    if (overlap) {
      var x1 =
        parseInt(parent.subContainers[0].style.left) - Math.ceil(overlap / 2);
      var x2 =
        parseInt(parent.subContainers[1].style.left) + Math.ceil(overlap / 2);
...
TPaint.prototype.getExtend = function (parent) {
  ...
  while ((cont1 = co1s.shift())) {
    ...
    while ((cont2 = co2s.shift())) {
      ...
      if (
        (cont1.__y >= cont2.__y && cont1.__y < cont2.__y + cont2.h) ||
        (cont2.__y >= cont1.__y && cont2.__y < cont1.__y + cont1.h)
      ) {
        var overlap12 =
          cont1.__x +
          cont1.w / 2 +
          painter.branchPadding -
          (cont2.__x - cont2.w / 2);
...
TPaint.prototype.treeInView = function () {
  var containerMain = this.treeVal.firstChild;
  if (containerMain.getBoundingClientRect) {
    ...
    if (winTreeRatio < 1) {
      ...
      document.getElementById('treeVal').style.transform =
        'scale(' + this.scale + ')';
    }
  }
...
document.forms[0].flaField.insertAtCaret = function (str) {
  if (document.selection) {
    ...
    var valu = this.value;
    this.value =
      valu.substring(0, startPos) + str + valu.substring(endPos, valu.length);
    this.focus();
...
function beginProof() {
  ...
  document.getElementById('treeVal').style.display = 'none';
  ...
  prover.onfinished = function (treeClosed) {
    ...
    self.painter = new TPaint(sTree, document.getElementById('treeVal'));
