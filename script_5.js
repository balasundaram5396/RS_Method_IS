  ...
  switch (node.fromRule) {
    ...
    case Prover.alpha: {
      ...
      if (!formula_node.equals(b1.normalize())) node.formula = b2;
        ...
        node.formula =
          parent.fromNodes[0] && parent.fromNodes[0] == from ? b2 : b1;
      }
      this.appendChild(parent, node);
      if (
        parent.fromNodes[0] &&
        parent.fromNodes[0] == from &&
        node.formula == b1
      ) {
    ...
    case Prover.delta: {
      ...
      var varBound = from.formula.sub
        ? from.formula.sub.variable
        : from.formula.variable;
...
STree.prototype.addInitNodes = function () {
  var branch =
    this.treeFV.closedBranches.length > 0
      ? this.treeFV.closedBranches[0]
      : this.treeFV.openBranches[0];
    ...
    if (varMatches) {
      for (var j = 0; j < varMatches.length; j++) {
        ...
        if (!meanings[fv]) {
          var sym =
            fv[0] == 'ζ'
              ? this.parser.getNewWorldName()
              : this.parser.getNewConstant();
          meanings[fv] = sym;
        }
        node.formula = node.formula.substitute(fv, meanings[fv]);
      }
    }
    var skterms = getSkolemTerms(node.formula);
    var separateTerms = skterms[0],
      universalTerms = skterms[1];
...
STree.prototype.createModel = function () {
  for (var i = 0; i < this.nodes.length; i++) {
    ...
    if (node.formula.predicate == this.parserFV.R) {
      node.formula.string =
        node.formula.terms[0] + this.parserFV.R + node.formula.terms[1];
    }
  }
};
...
STree.prototype.toString = function () {
  return (
    "<table><tr><td align='center' style='font-family:monospace'>" +
    getTree(this.nodes[0]) +
    '</td></tr></table>'
  );
  function getTree(node) {
    var recursionDepth = arguments[1] || 0;
    if (++recursionDepth > 50)
      return '<b>......<br>Reache MAX Recursion limit.</b>';
    var res =
      (node.used ? '.' : '') +
      node +
      (node.closedEnd ? '<br><br>-<br>' : '<br><br><br>');
    if (node.children[1])
      res +=
        "<table><tr><td align='center' valign='top' style='font-family:monospace; border-top:1px solid #999; padding:3px; border-right:1px solid #999'>" +
        getTree(node.children[0], recursionDepth) +
        "</td>\n<td align='center' valign='top' style='padding:3px; border-top:1px solid #999; font-family:monospace'>" +
        getTree(node.children[1], recursionDepth) +
        '</td>\n</tr></table>';
