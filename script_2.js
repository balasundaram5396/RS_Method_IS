...
Parser.prototype.getNewFunctionSymbol = function (ar) {
  return this.getNewSymbol('fghijklmn', ar + '-ary  symbol', ar);
};
...
Parser.prototype.stripAccessibilityClauses = function (formula) {
  if (formula.quantifier) {
    ...
    if (nmat == formula.matrix) return formula;
    return new QuantifiedFormula(formula.quantifier, formula.variable, nmat);
  }
  if (formula.sub1) {
    if (
      (formula.sub1.sub && formula.sub1.sub.predicate == this.R) ||
...
Parser.prototype.parseInput = function (str) {
  var parts = str.split('|=');
  if (parts.length > 2) {
    throw 'Multiple Turnstile not allowed';
  }
  var premises = [];
...
Parser.prototype.parseFormula = function (str) {
  ...
  if (retst) {
    ...
    var subStringsInParens = temp[1];
    var retst =
      nstr.match(/↔/) || nstr.match(/→/) || nstr.match(/∨/) || nstr.match(/∧/);
    if (retst) {
      var op = retst[0];
      ...
      if (!subFormulas[1]) {
        throw 'Argument missing- ' + op + ' in ' + str;
      }
  ...
  if (retst && retst.index == 0) {
    ...
    if (termstr) {
      ...
      if (predicate != this.R) this.isProp = false;
    } else {
  ...
  throw 'Not a proper formula. Please enter a valid formula and try again!';
};
...
Parser.prototype.parseTerms = function (str, boundVars) {
  ...
  do {
    ...
    if (!retst || retst.index != 0) throw 'Sequence of term required';
    ...
    if (str.indexOf('(') == 0) {
      ...
      this.registerExpression(retst[0], ar + '-ary symbol', ar);
