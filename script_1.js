...
Array.getArrayOfZeroes = function (length) {
  for (var j = 0, a = new Array(length); j < length; ) a[j++] = 0;
  return a;
};

Array.getArrayOfNumbers = function (length) {
  for (var j = 0, a = new Array(length); j < length; j++) a[j] = j;
  return a;
};
Array.prototype.copy = function () {
  var rs1 = [];
  for (var j = 0; j < this.length; j++) rs1[j] = this[j];
  return rs1;
};
Array.prototype.copyDeep = function () {
  var rs1 = [];
  for (var j = 0; j < this.length; j++) {
    if (this[j].isArray) rs1[j] = this[j].copyDeep();
    else rs1[j] = this[j];
  }
  return rs1;
};
Array.prototype.equals = function (arr1) {
  if (this.length != arr1.length) return false;
  for (var j = 0; j < this.length; j++) {
    if (this[j].isArray) {
      if (!arr1[j].isArray) return false;
      if (!this[j].equals(arr1[j])) return false;
    } else if (this[j] != arr1[j]) return false;
  }
  return true;
};
function Formula() {}
